...
REAPER_PLUGIN_DECLARE_APIFUNCS bool (*EnumPitchShiftModes)(int idx, const char **out); // returns FALSE when done, sets out to NULL if a mode is currently unsupported
...
REAPER_PLUGIN_DECLARE_APIFUNCS unsigned int (*PCM_Sink_Enum)(int id, const char **descstr);
